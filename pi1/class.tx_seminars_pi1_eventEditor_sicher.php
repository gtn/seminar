...
class tx_seminars_pi1_eventEditor extends tx_seminars_pi1_frontEndEditor {
	...
	/**
	 * Provides data items for the list of available accessories.
	 *
	 * @return array $items with additional items from the accessories table
	 *               as an array with the keys "caption" (for the title)
	 *               and "value" (for the UID)
	 */
...
/* gtn end */		
	...
	public function calcPrice($formData) {
		var_dump($formData);
		return 'CALC PRICE';
	}
...
//		var_dump($rawPost);
		...
		if ($rawPost['btncalculate'] and $this->getFormCreator()->oDataHandler->_allIsValid()) {		
			...
			if ($count_days<=0) 
				$count_days = 1;
			$price_room = 0;
			if ($rawPost['room_price_type'])
				$price_room += $this->conf['priceroom.'][$rawPost['room_price_type'].'.']['price'];
			if ($rawPost['room2_price_type'])
				$price_room += $this->conf['priceroom2.'][$rawPost['room2_price_type'].'.']['price'];
			if ($rawPost['kitchen_price_type'])
				$price_room += $this->conf['pricekitchen.'][$rawPost['kitchen_price_type'].'.']['price'];
				
			// accessories
			$in = '';
			if ($rawPost['accessories']) {
				foreach ($rawPost['accessories'] as $key=>$acc)
					$in .= $acc.',';
				$in = substr($in,0,-1);
				$accs = $GLOBALS['TYPO3_DB']->exec_SELECTgetSingleRow(
					'SUM(tx_seminars_accessories.tx_gtnseminarsext_price) as price_sum',
					'tx_seminars_accessories',
					'uid IN('.$in.')'
				);  
				$accessory_price = $accs['price_sum'];
			};
	...
	// List of room prices
	public function populateListRoomPrices($formData) {		
			$room_list = array();
	...
	// List of room2 prices
	public function populateListRoomPrices2($formData) {		
			$room_list = array();
